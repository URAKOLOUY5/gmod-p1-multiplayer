...
function ENT:Think()
        ...
        local portaltype = self:GetNWInt("Potal:PortalType",TYPE_Yellow)

        local glowcolor = Color( 255, 255, 0, 255 )
       
        if portaltype == TYPE_Red then
...
local nonlinkedYellow = surface.GetTextureID( "models/portals/portalstaticoverlay_pbody_1_close" )
local nonlinkedRed = surface.GetTextureID( "models/portals/portalstaticoverlay_pbody_2_close" )
local Yellowbordermat = surface.GetTextureID( "models/portals/portalstaticoverlay_pbody_1" )
local Redbordermat = surface.GetTextureID( "models/portals/portalstaticoverlay_pbody_2" )
...
function ENT:DrawPortalEffects( portaltype )
        ...
        cam.Start3D2D( origin, ang, res )
                ...
                if ( RENDERING_PORTAL or !self:GetNWBool( "Potal:Linked", false ) or !self:GetNWBool( "Potal:Activated", false )) then
                        ...
                        if portaltype == TYPE_Yellow then
						
								surface.SetTexture( nonlinkedYellow )
								
                        elseif portaltype == TYPE_Red then
						
                                surface.SetTexture( nonlinkedRed )
                ...
                if bordersenabled:GetBool() == true then                    
                        if portaltype == TYPE_Yellow then
						   if ( self:GetNWBool( "Potal:Linked", false ) or !self:GetNWBool( "Potal:Activated", false )) then
                                        ...
                                        surface.SetTexture( Yellowbordermat )
                           end
                        ...
                        elseif portaltype == TYPE_Red then
                             if ( self:GetNWBool( "Potal:Linked", false ) or !self:GetNWBool( "Potal:Activated", false )) then
                                        surface.SetTexture( Redbordermat )
                             end
                                surface.DrawTexturedRect( 0, 0, width / res , height / res )
	...
	if IsInFront( pos, self:GetRenderOrigin(), self:GetForward() ) and self:GetNWBool("Potal:Activated",false) then
		...
		render.SetStencilEnable( true )
			...
			cam.Start3D2D(self:GetRenderOrigin(),self:GetAngles(),1)
				...
				local ToRT = portaltype == TYPE_Yellow and texFSB or texFSB2
				...
				local color = Material("models/portals/portalstaticoverlay_pbody_1", "PortalRefract")
				if portaltype == TYPE_Red then
					color = Material("models/portals/portalstaticoverlay_pbody_2", "PortalRefract")
				end
...
hook.Add("PostDrawOpaqueRenderables","DrawPortalspbody", function()
...
function ENT:RenderPortal( origin, angles)
	if renderportals:GetBool() then
		...
		if IsValid( portal ) and self:GetNWBool( "Potal:Linked", false ) and self:GetNWBool( "Potal:Activated", false ) then
			...
			local oldrt_pbody = render.GetRenderTarget()
			...
			render.SetRenderTarget( oldrt_pbody ) 
		end
	end
end
...
hook.Add( "ShouldDrawLocalPlayer", "Portal.ShouldDrawLocalPlayerpbody", function()
...
hook.Add( 'PostDrawEffects', 'PortalSimulation_PlayerRenderFixpbody', function()
	cam.Start3D( EyePos(), EyeAngles() )
	cam.End3D()
end)

CreateClientConVar("portal_debugmonitor", 0, false, false)
hook.Add( "HUDPaint", "Portal.YellowMonitor", function( w,h )
	if GetConVarNumber("portal_debugmonitor") == 1 and GetConVarNumber("sv_cheats") == 1 then
		...
		for k, v in ipairs( ents.FindByClass( "prop_portal_pbody" ) ) do
		  // debug monitor
			if view and v:GetNWInt("Potal:PortalType", TYPE_Yellow) == TYPE_Yellow then
...
/*------------------------------------
        GetMotionBlurValues()
------------------------------------*/
hook.Add( "GetMotionBlurValues", "Portal.GetMotionBlurValuespbody", function( x, y, fwd, spin )
        if RENDERING_PORTAL then
                return 0, 0, 0, 0
        end
end )

hook.Add( "PostProcessPermitted", "Portal.PostProcessPermittedpbody", function( element )
...
hook.Add( "RenderScreenspaceEffects", "Portal.RenderScreenspaceEffectspbody", function()
