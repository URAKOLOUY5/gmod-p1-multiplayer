...
ENT.Linked = nil
ENT.PortalType = TYPE_Blue_Light
ENT.Activated = false
ENT.KeyValues = {}
...
function ENT:Initialize( )
	...
	-- self:SetModel( "models/portals_atlas/portal1_renderfix.mdl" )
...
function ENT:CleanMeUp()
	...
	PrecacheParticleSystem("portal_2_close_atlas")
	PrecacheParticleSystem("portal_1_close_atlas")
	...
	if self.PortalType == TYPE_Blue_Light then
		ParticleEffect("portal_1_close_atlas",pos,ang,nil)
		elseif self.PortalType == TYPE_Blue_Dark then
		ParticleEffect("portal_2_close_atlas",pos,ang,nil)
	end
...
function ENT:SuccessEffect()
	...
	ParticleEffect("portal_"..self.PortalType.."_success_atlas",pos,ang,self)
...
function ENT:StartTouch(ent)
	...
	if ent:GetClass() == "projectile_portal_ball_atlas" then ent:Remove() return end
...
hook.Add("EntityFireBullets", "BulletPortingatlas", BulletHook)
...
hook.Add("SetupPlayerVisibility", "Add portalPVS pbody", function(ply,ve)
	for k,self in pairs(ents.FindByClass("prop_portal_atlas"))do
		...
		-- if self:GetNWInt("Potal:PortalType") == TYPE_Blue_Dark then
