...
local leftpos1 = {x = -2, y = -5}
local rightpos1 = {x = 10, y = 17}
	...
	if GetConVarNumber("portal_crosshair") >= 2 then
		...
		elseif lastPort == TYPE_ORANGE then
			...
			surface.DrawTexturedRect(
				cX + rightpos1.x + 38,
				cY + rightpos1.y - (sizeSmall1.h / 2) + 21,
				sizeSmall1.w,
				sizeSmall1.h
			)
		end
...
local VElements = {
	["BodyLight"] = {
		type = "Sprite",
		sprite = "sprites/portalgun_light2",
		bone = "ValveBiped.Base",
		rel = "",
		pos = Vector(0.25, -5.45, 10.5),
		size = {x = 0.018, y = 0.018},
	...
	["BodyLight"] = {
		type = "Sprite",
		sprite = "sprites/portalgun_light2",
		bone = "ValveBiped.Front_Cover",
		rel = "",
		pos = Vector(-0.101, -2.401, -6.737),
		size = {x = 0.018, y = 0.018},
		color = Color(255, 255, 255, 255),
		nocull = false,
		additive = true,
...
local WElements = {
	...
	["BodyLight1"] = {
		type = "Sprite",
		sprite = "sprites/portalgun_effects",
		bone = "ValveBiped.Bip01_R_Hand",
		rel = "",
		pos = Vector(12.541, 0.324, -5.699),
		size = {x = 0.04, y = 0.04},
		color = Color(255, 255, 255, 255),
		nocull = false,
		additive = true,
...
if GetConVarNumber("portal_beta_borders") >= 1 then
	...
	WElements = {
		["BodyLight"] = {
			type = "Sprite",
			sprite = "sprites/portalgun_light",
			bone = "ValveBiped.Bip01_R_Hand",
			rel = "",
			pos = Vector(6.344, 0.324, -6.737),
			size = {x = 0.03, y = 0.03},
...
function SWEP:DrawWeaponSelection(x, y, wide, tall, alpha)
	...
	local w,
		h = surface.GetTextSize(self.WepSelectLetter)
	...
	function()
		...
		if not IsValid(ent) then
			...
			function self.ViewModelOverride.PreDraw(vm)
				...
				local pos,
					ang = self:CalcViewModelView(vm, oldorigin, EyeAngles(), vm:GetPos(), vm:GetAngles())
				return pos, ang
			end
		end
...
local GravityLight,
	GravityBeam = Material("sprites/light_glow02_add"), Material("particle/bendibeam")
...
function SWEP:DrawPickupEffects(ent)
	...
	for k, v in pairs(GravitySprites) do
		...
		local pos,
			ang = Vector(0, 0, 0), Angle(0, 0, 0)
		local m = ent:GetBoneMatrix(bone)
		if (m) then
			pos,
				ang = m:GetTranslation(), m:GetAngles()
		end
	...
	local endpos,
		ang = bone:GetTranslation(), bone:GetAngles()
...
function SWEP:DoPickupAnimations(vm)
	...
	local toCarry,
		toCarryLength = vm:LookupSequence("idle_to_carrying")
	local carry,
		carryLength = vm:LookupSequence("idle_carrying")
...
function SWEP:ViewModelDrawn(vm)
	...
	for k, v in pairs(VGravitySprites) do
		...
		if (IsValid(self.Owner) and self.Owner:IsPlayer() and vm == self.Owner:GetViewModel() and self.ViewModelFlip) then
			ang.r = -ang.r -- Fixes mirrored models
		end
	...
	for k, name in pairs(self.VElements) do
		...
		local pos,
			ang = self:GetBoneOrientation(self.VElements, v, vm)
...
function SWEP:DrawWorldModel()
	...
	for k, name in pairs(self.wRenderOrder) do
		...
		if (v.bone) then
			pos,
				ang = self:GetBoneOrientation(self.WElements, v, bone_ent)
		else
			pos,
				ang = self:GetBoneOrientation(self.WElements, v, bone_ent, "ValveBiped.Bip01_R_Hand")
		end
...
function SWEP:GetBoneOrientation(basetab, tab, ent, bone_override)
	...
	if (tab.rel and tab.rel ~= "") then
		...
		pos,
			ang = self:GetBoneOrientation(basetab, v, ent)
...
local sin,
	...
	pi,
	clamp,
	min = math.sin, math.abs, math.pi, math.Clamp, math.min
