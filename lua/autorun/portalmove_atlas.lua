if( SERVER ) then
    AddCSLuaFile( "portalmove_atlas.lua" );
end
...
if SERVER then
	hook.Add("DoPlayerDeath", "Remove Portals On Death Atlas", function(victim)
		local Blue_Lightportal = victim:GetNWEntity( "Portal:Blue_Light" )
        local Blue_Darkportal = victim:GetNWEntity( "Portal:Blue_Dark" )
       
        for k,v in ipairs( ents.FindByClass( "prop_portal_atlas" ) ) do
   
			if v == Blue_Lightportal or v == Blue_Darkportal and v.CleanMeUp then
				v:CleanMeUp()
			end
