...
function ENT:Think()
			...
			if v.Ent1 == self then
				if v.Ent2:GetModel() == "models/portal_custom/metal_ball_custom.mdl" then
					if IsValid(self.Owner) then
						LIB_APERTURECONTINUED.ACHIEVEMENTS:AchievAchievement(self.Owner, "inventor")
					end
				end
	...
	if IsValid(self.TurretPersonal) then
		return
	end
	if not self.TurretDifferent then
		...
		for k, v in pairs(entities) do
			...
			if string.find(model, "cake") then
				local ply = v.Player
				if
					v.OnDieFunctions and v.OnDieFunctions.GetCountUpdate and v.OnDieFunctions.GetCountUpdate.Args and
						v.OnDieFunctions.GetCountUpdate.Args[1]
				 then
					ply = v.OnDieFunctions.GetCountUpdate.Args[1]
				end
				if IsValid(ply) then
					LIB_APERTURECONTINUED.ACHIEVEMENTS:AchievAchievement(ply, "im_different")
				end
				self:MakeDifferent()
				v:SetPos(Vector())
				v:Remove()
	...
	if not self.TurretSong and self:GetTurretState() == TURRET_STATE_IDLE then
		...
		for k, v in pairs(entities) do
			if
				v ~= self and v:GetClass() == "npc_portal_turret_floor" and v.TurretDifferent and
					v:GetTurretState() == TURRET_STATE_IDLE and
					not v.TurretSong
			 then
...
function ENT:StopSing()
	if not self.TurretSong then
		return
	end
	if not self.TurretBundle then
		return
	end
	for k, v in pairs(self.TurretBundle) do
		if IsValid(v) then
...
function ENT:SingInit(turretInx)
	...
	if turretInx == 1 then
		self:PlaySequence("3penny_hi", 1.0)
	elseif turretInx == 2 then
		self:PlaySequence("3penny_lo", 1.0)
	elseif turretInx == 3 then
		self:PlaySequence("3penny_mid", 1.0)
	elseif turretInx == 4 then
		self:PlaySequence("3penny_perc", 1.0)
	end
