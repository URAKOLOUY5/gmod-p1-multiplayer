...
function ENT:Initialize( )
	...
	-- self:SetModel( "models/portals_pbody/portal1_renderfix.mdl" )
...
function ENT:CleanMeUp()
	...
	PrecacheParticleSystem("portal_2_close_pbody")
	PrecacheParticleSystem("portal_1_close_pbody")
	...
	if self.PortalType == TYPE_Yellow then
		ParticleEffect("portal_1_close_pbody",pos,ang,nil)
		elseif self.PortalType == TYPE_Red then
		ParticleEffect("portal_2_close_pbody",pos,ang,nil)
	end
...
function ENT:SuccessEffect()
	...
	ParticleEffect("portal_"..self.PortalType.."_success_pbody",pos,ang,self)
...
function ENT:StartTouch(ent)
	...
	if ent:GetClass() == "projectile_portal_ball_pbody" then ent:Remove() return end
...
hook.Add("EntityFireBullets", "BulletPortingpbody", BulletHook)
...
hook.Add("SetupPlayerVisibility", "Add portalPVS pbody", function(ply,ve)
	for k,self in pairs(ents.FindByClass("prop_portal_pbody"))do
		...
		-- if self:GetNWInt("Potal:PortalType") == TYPE_Red then
