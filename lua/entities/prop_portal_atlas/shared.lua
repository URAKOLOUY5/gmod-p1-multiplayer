...
function ENT:SetUpEffects(int)
	...
	ent:SetPos(pos)
	ent:SetAngles(ang)
	if int == TYPE_Blue_Light then
		ent:SetKeyValue( "effect_name", "portal_1_edge_atlas")
	elseif int == TYPE_Blue_Dark then
...
function ENT:GetOpposite() --Don't think this is being used..? Gets the portal type that it would need to be linked too
	if self.PortalType == TYPE_Blue_Light then
		return TYPE_Blue_Dark
	elseif self.PortalType == TYPE_Blue_Dark then
		return TYPE_Blue_Light
	end
end
...
hook.Add( "PhysgunPickup", "NoPickupPortalsatlas", PlayerPickup )
hook.Add( "GravGunPickupAllowed", "NoPickupPortalsatlas", PlayerPickup )
hook.Add( "GravGunPunt", "NoPickupPortalsatlas", PlayerPickup )