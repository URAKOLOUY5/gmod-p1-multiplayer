...
if SERVER then
	function ENT:ShootRocket(pos, ang)
		...
		if IsValid(rocket:GetPhysicsObject()) then
			rocket:GetPhysicsObject():SetVelocity(ang:Forward() * 600)
		end
...
function ENT:TransfortTurret()
	...
	if rootLR then
		self:ManipulateBoneAngles(rootLR, Angle(angle.yaw, 0, 0))
	end
	if rootUD then
		self:ManipulateBoneAngles(rootUD, Angle(0, -angle.pitch, 0))
	end
	if arm1Bone then
		self:ManipulateBoneAngles(arm1Bone, Angle(0, ang1 * -90, 0))
	end
	if arm2Bone then
		self:ManipulateBoneAngles(arm2Bone, Angle(0, ang1 * 90 - ang2 * 135, 0))
	end
	if arm3Bone then
		self:ManipulateBoneAngles(arm3Bone, Angle(0, ang1 * 35 + ang2 * 135, 0))
	end
	if arm4Bone then
		self:ManipulateBoneAngles(arm4Bone, Angle(0, ang1 * -35, 0))
	end
end
...
function ENT:Think()
	...
	if self:GetEnable() then
		target,
			center = LIB_APERTURECONTINUED:FindClosestAliveInSphereIncludingPortalPassages(pos, 2000)
	end
	if IsValid(target) then
		local _,
			trace = LIB_APERTURECONTINUED:GetAllPortalPassages(pos, (center - pos), nil, self)
		if not IsValid(trace.Entity) or trace.Entity ~= target then
			target = nil
		end
	end
	...
	elseif turretState == TURRET_STATE_SEARCH then
		...
		if not timer.Exists("TA:ChangeTurretLooking" .. self:EntIndex()) then
			timer.Create(
				"TA:ChangeTurretLooking" .. self:EntIndex(),
				5,
				1,
				function()
				end
	...
	elseif turretState == TURRET_STATE_LOCKING then
		if IsValid(target) then
			...
			local _,
				traceBeam = LIB_APERTURECONTINUED:GetAllPortalPassagesAng(pos, wang, 0, self)

			self.TurretLookingPoint = center

			if
				IsValid(traceBeam.Entity) and traceBeam.Entity == target and
					not timer.Exists("TA:RTurretCooldown" .. self:EntIndex())
			 then
				...
				timer.Simple(
					...
					function()
						...
						local bpos,
							bang = self:GetBonePosition(laserBeamBone)
						self:SetSkin(2)
	...
	elseif turretState == TURRET_STATE_SHOOT then
		if not IsValid(self.TurretLaunchedRocket) then
			...
			timer.Create(
				"TA:RTurretCooldown" .. self:EntIndex(),
				2,
				1,
				function()
				end
...
function ENT:Drawing()
	...
	local turretState = self:GetTurretState()
	if turretState == TURRET_STATE_DEPLOY then
		return
	end
	if turretState == TURRET_STATE_RETRACT then
		return
	end
	if turretState == TURRET_STATE_PREPARE_RETRACT then
		return
	...
	local laserBeamBone = self:LookupBone("Gun_casing")
	local pos,
		ang = self:GetBonePosition(laserBeamBone)
